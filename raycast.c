...
/*   Updated: 2022/02/09 16:50:30 by tmartial         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float init_raycast(t_ray *ray, t_data *data)
{
    ...
    while (!bTileFound && fdistance < fmaxdistance)
    {
        ...
        if(bTileFound == 1)
        {
            intersection_x = ray->start_x + ray->dir_x * fdistance;
            intersection_y = ray->start_y + ray->dir_y * fdistance;
            write(1, "Hello\n", 6);
            return(sqrt (pow(intersection_x,2) + pow(intersection_y,2)) );
        }
    }
    return(0);
}