...
/*   Updated: 2022/02/10 16:45:32 by tmartial         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void make_player(t_data *data)
{
	...
	//draw_line(data, (int)(data->x * 100) + 5, (int)(data->y * 100) + 5, 700, 700);
	i = 0;
	data->direction = PI;
...
void    draw_line2(t_data *data, int x0, int y0, int x1, int y1)
{
    ...
    while (1)
    {
        my_mlx_pixel_put(data, x0, y0, 0x0000FFFF);//mauve 0x006A0DAD
        e2 = 2 * err;
        if (e2 >= dy)
        {
            if (x0 == x1)
